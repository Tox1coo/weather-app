<template>
  <div class="forecastlist">
    ...
    <button
      :disabled="currentDay === Math.floor(forecastInfo?.length / 2)"
      :class="{
        'arrow-active': currentDay < Math.floor(forecastInfo?.length / 2),
        'arrow-disabled': currentDay === Math.floor(forecastInfo?.length / 2),
      }"
      @click="nextDayTime()"
      class="arrow-next"
    >
